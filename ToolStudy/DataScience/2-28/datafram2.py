...
'''
# 通过使用字典来创建数据框，创建一个8行四列的数据框，每一列的名字是字典的key，值是字典的value
if True:
...
1) dtypes: to get the datatype for each column，返回一个Series对象，其值为每列的类型
2) describe(): useful for seeing basic statistics of the dataframe's numerical
   columns，返回一个数据框，获得数值列的一些信息（比如最大值，中位数等等）
3) head（）: displays the first five rows of the dataset  获得前五行
4) tail（）: displays the last five rows of the dataset 获得后5行
'''
