...
def getWordScore(word, n):
    ...
    word = word.lower()
    # 附加得分
    addtion = 0 if len(word) != n else 50
    summation = sum([SCRABBLE_LETTER_VALUES[letter] for letter in word]) * len(word)
    return summation + addtion
...
def updateHand(hand, word):
    ...
    myhand = hand.copy()
    for letter in word:
        if myhand.get(letter) and myhand.get(letter) > 0:
            myhand[letter] -= 1
    return myhand
    ...
    while calculateHandlen(hand) > 0:
        ...
        if word == ".":
            ...
            if not isValidWord(word, hand, wordList):
                ...
                total += points
                print "%s earned %d points. Total: %d points" % (word, points, total)
