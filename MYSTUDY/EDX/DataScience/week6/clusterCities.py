...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        points = []
        for p1, p2 in zip(c1.members(), c2.members()):
            points.append(p1)
            points.append(p2)
    ...
    def findClosest(self, linkage):
        ...
        result_tuple = None
        mindist = None
        for index, closter in enumerate(self.members[:]):
            for closer_other in self.members[index+1:]:
                if result_tuple is None:
                    result_tuple = (closter, closer_other)
                if mindist is None:
                    mindist = linkage(closter, closer_other)
                dist = linkage(closter, closer_other)
                if mindist > dist:
                    mindist = dist
                    result_tuple = (closter, closer_other)
        return result_tuple
    ...
    def mergeOne(self, linkage):
        ...
        merged = self.findClosest(linkage)
        self.mergeClusters(*merged)
        return merged
